...
$(document).ready(function() {
	...
	Crafty.scene("title", function() {
		...
		Crafty.e("2D, canvas, title").attr({x: Crafty.viewport.width / 2 - 215});
		button = Crafty.e("2D, canvas, mouse").attr({x: Crafty.viewport.width / 2 - 75, y: 290, w: 142, h: 74}).bind("click", function(e) {
			console.log("CLICK");
			Crafty.scene("main");
		});
	});
	...
	Crafty.scene("main", function() {
		...
		//Create the player
		var player = Crafty.e("2D, player, canvas, gravity, controls, twoway, collision, animate, audio, health");
		...
		//Generate some doors
		for(var i = 1; i <= 10; i++) {
			var door = Crafty.e("2D, door, canvas, explosive");
			door.attr({x: 250, y: i*80, z: i});
			
			var red = Crafty.e("2D, reddoor, canvas, explosive");
		...
		var barrier = Crafty.e("barrier, canvas, image").barrier(0,544,Crafty.viewport.width / 2,20, player).image("images/girder.png", "repeat-x");
		barrier.north.addComponent("floor");
		
		floor1 = Crafty.e("barrier, canvas, image").barrier(0, 224, Crafty.viewport.width / 2, 20, player).image("images/girder.png", "repeat-x");
		floor1.north.addComponent("floor");
		
		var floor3 = Crafty.e("barrier, canvas, image").barrier(Crafty.viewport.width / 2 + 50, 224, Crafty.viewport.width / 2, 20, player).image("images/girder.png", "repeat-x");
		floor3.north.addComponent("floor");
		
		var floor4 = Crafty.e("barrier, canvas, image").barrier(Crafty.viewport.width / 2 + 50, 544, Crafty.viewport.width / 2, 20, player).image("images/girder.png", "repeat-x");
		...
		var elevator = Crafty.e("2D, canvas, color, elevator").color("rgb(200,200,200)").attr({x: Crafty.viewport.width / 2, y:0, w: 50, h: 80});
		...
		var rope = Crafty.e("2D, canvas, color").color("rgb(30,30,30)").attr({x: elevator.x + 23, y: Crafty.viewport.height * -1, h: Crafty.viewport.height, w: 2});
		var rope2 = Crafty.e("2D, canvas, color").color("rgb(60,60,60)").attr({x: elevator.x + 28, y: Crafty.viewport.height * -1, h: Crafty.viewport.height, w: 1});
		...
		for(var k=1; k <= 10; k++) {
			var light = Crafty.e("2D, canvas, light, collision, audio").attr({x: 100 * k, y: 244}).collision("bullet", function(e) {
